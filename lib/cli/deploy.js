...
var doc = "Usage:\n" +
	...
	"	--coverage-report=<file> Export coverage to CSV file.\n" +
...
SubCommand.prototype.sumCodeCoverage = function(codeCoverage) {
	if (!Array.isArray(codeCoverage)) {
		codeCoverage = [codeCoverage];
	}
	var total = codeCoverage.filter(function(coverage) {
...
SubCommand.prototype.exportCodeCoverage = function(codeCoverage) {
	if (!Array.isArray(codeCoverage)) {
		codeCoverage = [codeCoverage];
	}
	var lines = [];
	lines.push(["Type", "Name", "Percent", "LinesNotCovered", "Lines"].join(","));
	codeCoverage.filter(function(coverage) {
		// exclude empty classes
		return coverage.numLocations && parseInt(coverage.numLocations) > 0;
		...
		lines.push([
			coverage.type,
			coverage.name,
			coveragePercentage.toLocaleString('en-us'),
	...
	remote.connect(function(loginErr, conn) {
		...
		conn.metadata.deploy(zipStream, self.deployOpts).complete({
			...
			if (res.details && res.details.runTestResult && res.details.runTestResult.codeCoverage) {
				console.log('Overall Code Coverage: ' + self.sumCodeCoverage(res.details.runTestResult.codeCoverage).toLocaleString('en-us', {
					style: 'percent'
				}));
				if (self.opts['--coverage-report']) {
					var coverageFilePath = path.resolve(self.opts['--coverage-report']);
					fs.writeFileSync(coverageFilePath, self.exportCodeCoverage(res.details.runTestResult.codeCoverage));
					console.log('Exported coverage to ' + chalk.cyan(path.relative(proc.cwd, coverageFilePath)));
				}
			}
			if (res.id) {
